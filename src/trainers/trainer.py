...
class Trainer:
    ...
    def prepare_training(self):
        ...
        self.steps_per_epoch = len(self.dataloaders['train'])*self.cfg.batch_size
    ...
    def train_one_epoch(self):
        ...
        for x in self.dataloaders['train']:
        ...
        if self.cfg.use_tensorboard:
            self.summarizer.add_scalar("epoch_loss/train", self.epoch_train_losses[-1], self.epoch)
            if self.cfg.use_scheduler:
    ...
    def validate_one_epoch(self):
        ...
        val_losses = [
            self.model.batch_loss(ensure_device(x, self.device)).item()
        ...
        if self.cfg.use_tensorboard:
            self.summarizer.add_scalar("epoch_loss/val", self.epoch_val_losses[-1], self.epoch)
